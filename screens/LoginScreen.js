...
import { AuthContext } from '../store/AuthContext'; // ✅ đúng path
...
export default function LoginScreen({ navigation }) {
    ...
    const handleLogin = async () => {
        ...
        setLoading(true);
        // login(email, password) trả về { success, user } hoặc { success, error }
        const result = await login(email.trim(), password);
        setLoading(false);

        if (!result.success) {
            // result.error là chuỗi tiếng Việt luôn (từ AuthContext của bạn)
            Alert.alert('Đăng nhập thất bại', result.error);
        }
        // Nếu success → AuthContext set user → Navigator tự chuyển màn hình
    };
...
const styles = StyleSheet.create({
    ...
    headerLabel: { alignSelf: 'flex-start', fontSize: 14, color: '#555', marginBottom: 16 },
    logoBox: {
    ...
    appTitle: { fontSize: 32, fontWeight: '900', color: GREEN, textAlign: 'center', lineHeight: 38, letterSpacing: 1, marginBottom: 8 },
    appSubtitle: { fontSize: 13, color: '#666', marginBottom: 28, textAlign: 'center' },
    card: {
    ...
    loginBtn: { backgroundColor: GREEN_LIGHT, borderRadius: 30, height: 52, justifyContent: 'center', alignItems: 'center', marginTop: 20, shadowColor: GREEN, shadowOpacity: 0.35, shadowRadius: 8, elevation: 4 },
    ...
    socialBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#e0e0e0', borderRadius: 12, height: 46, gap: 8, backgroundColor: '#fafafa' },
