...
import { getFieldImage } from '../services/fieldImages'; // ✅ dùng chung
...
export default function FieldListScreen({ route, navigation }) {
    ...
    const filterFields = (data, sport) => {
        setFiltered(sport === 'Tất cả' ? data : data.filter(f => f.sport === sport));
    };
        ...
        <View style={styles.wrapper}>
            ...
            <View style={styles.filterWrapper}>
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        >
                            <Text style={[styles.filterText, selectedSport === item && styles.filterTextActive]}>
                                {item}
                            </Text>
                        </TouchableOpacity>
                    )}
            ...
            <FlatList
                ...
                renderItem={({ item: field }) => {
                    ...
                    const imageSource = getFieldImage(field.image); // ✅ key riêng từng sân
                    return (
                        <TouchableOpacity
                        ...
                        >
                            <View style={styles.imageBox}>
                                {imageSource ? (
                                    ...
                                    <View style={[styles.imagePlaceholder, { backgroundColor: sportColor + '22' }]}>
                                        <Text style={{ fontSize: 48 }}>🏟️</Text>
                                    </View>
                                )}
                            ...
                            <View style={styles.fieldBody}>
                                ...
                                <View style={styles.fieldBottomRow}>
                                    <View>
                                        ...
                                        <Text style={[styles.fieldPrice, { color: sportColor }]}>
                                            {field.price.toLocaleString('vi-VN')}đ
                                            <Text style={styles.priceUnit}>/giờ</Text>
                                        </Text>
                                    </View>
                                    <TouchableOpacity
                                        style={[styles.bookBtn, { backgroundColor: sportColor }]}
                                        onPress={() => navigation.navigate(detailRoute, { field })}
                                    >
...
const styles = StyleSheet.create({
    ...
    filterWrapper: { backgroundColor: '#fff', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', elevation: 2 },
    filterRow: { paddingHorizontal: 16, gap: 8 },
    filterChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#f4f6f8', borderWidth: 1, borderColor: '#e0e0e0' },
    filterChipActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
    filterText: { fontSize: 13, color: '#888', fontWeight: '600' },
    filterTextActive: { color: '#fff' },
    listContainer: { padding: 16, gap: 14 },
    fieldCard: { backgroundColor: '#fff', borderRadius: 18, overflow: 'hidden', elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8 },
    imageBox: { position: 'relative' },
    fieldImage: { width: '100%', height: 160 },
    imagePlaceholder: { height: 160, justifyContent: 'center', alignItems: 'center' },
    sportBadge: { position: 'absolute', top: 12, left: 12, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20 },
    ...
    fieldTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
    ...
    fieldPrice: { fontSize: 17, fontWeight: '800' },
    priceUnit: { fontSize: 12, fontWeight: '500', color: '#aaa' },
    bookBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12 },
